...
# Assuming these files contain the class definitions and not global instances
...
class ErrorListener:
    ...
    async def start(self):
        ...
        self.logger.info(
            "Error Listener is now active and listening to the Event Bus."
        )
    ...
    async def on_error_received(self, event: Event):
        ...
        try:
            ...
            self.logger.warning(
                f"🚨 New error intercepted from source '{event.source}'!"
            )
            ...
            print(
                f"🛠️ [Self-Healing] Analyzing failure in function: {parsed_report.get('function')}"
            )

            # 3. Trigger the Auto-Fixer!
            # Background task prevents blocking the Event Bus queue
            asyncio.create_task(self.fixer.attempt_fix(parsed_report))

        except Exception as e:
            self.logger.error(
                f"Critical failure inside ErrorListener itself: {e}"
            )
...
# Global instance to be imported in lifecycle_manager.py
