...
class ContextValidator:
    ...
    async def validate_action_context(self, intent: str, current_context: dict):
        ...
        # -----------------------------
        # 1️⃣ File Operation Safety
        # -----------------------------
        if "file" in intent:
            target_path = state.get("target_path", "")
            is_admin = state.get("is_admin", False)

            # System directories should not be modified without admin
            if not is_admin and ("/etc/" in target_path or "/usr/" in target_path):
                msg = f"Blocked: Insufficient permissions to modify {target_path}"
                self.logger.warning(msg)
                return False, msg

        # -----------------------------
        # 2️⃣ UI Operation Safety
        # -----------------------------
        ...
        # -----------------------------
        # 4️⃣ Shell / Dangerous Commands
        # -----------------------------
        if intent.startswith("run_shell") or intent in ["delete_file", "install_package"]:
