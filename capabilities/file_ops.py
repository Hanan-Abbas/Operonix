...
CAPABILITY_METADATA: dict[str, dict] = {
    ...
    "open_folder":  {"tool": "shell_tool", "method": "command"},
    "open_file":    {"tool": "shell_tool", "method": "command"},
    "find_file":    {"tool": "shell_tool", "method": "command"},
}
...
async def open_folder(context: dict, args: dict) -> dict:
    """
    Open a folder in the system file manager.
    args: {path, folder_name, location}
    Uses xdg-open (Linux), explorer (Windows), open (macOS).
    """
    import subprocess
    import platform
    path = _resolve_path(args, context)

    # If path doesn't exist, search for it by name
    if not os.path.isdir(path):
        folder_name = args.get("folder_name") or args.get("name") or \
                      args.get("dir_name") or os.path.basename(path)
        cwd = context.get("cwd") or os.getcwd()
        # Search in cwd and common locations
        for search_root in [cwd, str(Path.home())]:
            try:
                result = subprocess.run(
                    ["find", search_root, "-maxdepth", "5",
                     "-type", "d", "-iname", folder_name],
    ...
    if not os.path.isdir(path):
        return {"success": False, "result": f"Folder not found: {path}", "intent": "open_folder"}
    ...
    try:
        ...
        return {"success": True, "result": f"Opened folder: {path}", "intent": "open_folder"}
    except Exception as exc:
        return {"success": False, "result": str(exc), "intent": "open_folder"}


async def open_file(context: dict, args: dict) -> dict:
    """Open a file with its default application.  args: {path, file_name}"""
    import subprocess
    import platform
    path = _resolve_path(args, context)
    if not os.path.exists(path):
        return {"success": False, "result": f"File not found: {path}", "intent": "open_file"}
    try:
        os_name = platform.system()
        if os_name == "Linux":
            subprocess.Popen(["xdg-open", path])
            ...
            subprocess.Popen(["open", path])
        return {"success": True, "result": f"Opened: {path}", "intent": "open_file"}
    except Exception as exc:
        return {"success": False, "result": str(exc), "intent": "open_file"}


async def find_file(context: dict, args: dict) -> dict:
    """Search for a file or folder by name.  args: {name, search_root, maxdepth}"""
    import subprocess
    name = args.get("name") or args.get("file_name") or args.get("folder_name", "")
    cwd = context.get("cwd") or os.getcwd()
    search_root = args.get("search_root") or cwd
    maxdepth = str(args.get("maxdepth", 6))
    try:
        result = subprocess.run(
            ["find", search_root, "-maxdepth", maxdepth, "-iname", f"*{name}*"],
            capture_output=True, text=True, timeout=5
        )
        hits = [h for h in result.stdout.strip().splitlines() if h]
        if hits:
            return {"success": True, "result": hits, "intent": "find_file"}
        return {"success": False, "result": f"Nothing found matching '{name}'", "intent": "find_file"}
    except Exception as exc:
        return {"success": False, "result": str(exc), "intent": "find_file"}
